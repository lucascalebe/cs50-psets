...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        {
            //GETTING AVERAGE
            rgb_gray = round((image[j][i].rgbtBlue + image[j][i].rgbtGreen + image[j][i].rgbtRed) / 3.000);

            //SETTING TO ALL COLOR THE SAME VALUE
            image[j][i].rgbtBlue = rgb_gray;
            image[j][i].rgbtGreen = rgb_gray;
            image[j][i].rgbtRed = rgb_gray;
        }
    }
}

// FUNCTION TO STOP AT MAX VALUE
int limit(int RGB)
{
    if (RGB > 255)
    {
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        {
            array[0] = image[j][i].rgbtBlue;
            array[1] = image[j][i].rgbtGreen;
            array[2] = image[j][i].rgbtRed;
            ...
            image[j][width - i - 1].rgbtBlue = array[0];
            image[j][width - i - 1].rgbtGreen = array[1];
            image[j][width - i - 1].rgbtRed = array[2];
        }
    }
}
